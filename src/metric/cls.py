...
class ClsMetric(Metric):
    def __init__(
        ...
        """
            ...
            Args:
                thresh: (only useful in binary classification), confusion matrix thresh, if use float will not need to save probs and labels
                    ...
                    - 'f1_score': will use thresh that has best f1_score(todo)
                    - 'roc': will use thresh that has best tpr - fpr(todo)
        ...
        self.probs = torch.tensor([], dtype=torch.float) # num_classes == 2: (B,), else: (B, C)
        self.labels = torch.tensor([], dtype=torch.long) # (B,)
    ...
    def reset(self):
        self._matrix = torch.zeros((self.num_classes, self.num_classes), dtype=torch.long)
